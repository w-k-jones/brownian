...
History:
    ...
    6/12/2016: LM - Added sawtooth function with trigger wall on tooth side.
                    Assimilated with rest of code
                    Modified walls class to wall_shape & sorted wall & normal vectors
                    Added wall_modifier inhereted class as wall-type setter
...
"""
#nd = 2 #redundant b/c defined later
...
class wall_shape:
    ...
    def get_v(self):
        ...
        for i in np.arange(self.n):
            if i < self.n - 1:
                ...
                self.v[i] = self.v[i] / (np.sum(self.v[i]**2))**0.5
            elif i == self.n - 1:
                self.v[i] = self.co[0] - self.co[i]
                self.v[i] = self.v[i] / (np.sum(self.v[i]**2))**0.5
        
        #self.v = self.v / (np.sum(self.v**2, axis=1))**0.5
        #^doesn't work b/c sum is row vector now column, so messes order up (fixed above)
        
        return self.v

    """Get perpendicular normal vector for each wall (2d)"""
    def get_n(self, side=1):
        """side gives wall side of normal"""
        
        self.norm = np.full([self.n,self.nd],np.nan)
        self.norm2 = np.full([self.n,self.nd],np.nan)
        
        if self.nd == 2:
            self.norm[:,0], self.norm[:,1] = self.co[:,1], self.co[:,0]
            if side == 1:
                self.norm[:,1] = self.norm[:,1] * -1
            elif side == -1:
                self.norm[:,0] = self.norm[:,0] * -1
                
            for i in np.arange(self.n):
                if i < self.n - 1:
                    self.norm2[i] = self.norm[i+1] - self.norm[i]
                elif i == self.n - 1:
                    self.norm2[i] = self.norm[0] - self.norm[i]
                    
        for i in np.arange(self.n):
            self.norm2[i] = self.norm2[i] / (np.sum(self.norm2[i]**2))**0.5
    ...
    """
    #don't know what this does yet^ but think i've made it so we don't need it
...
""" 
Initialise walls and set type for each wall = walls.type(index)
Note that sawtooth returns .x_v and y_v as trigger-wall coordinates
"""

coordinate_array = np.array([[0,0],[0,1],[1,1],[1,0]])

walls = wall_modifier(coordinate_array)
walls.flat(0)
walls.sawtooth(1,5)
walls.flat(2)
walls.flat(3)
